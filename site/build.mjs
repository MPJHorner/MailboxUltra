// MailBox Ultra documentation site builder.
...
const SITE_URL = process.env.SITE_URL || 'https://mpjhorner.github.io/MailboxUltra';
const BASE = process.env.BASE_URL || '/MailboxUltra';
const REPO = 'https://github.com/MPJHorner/MailboxUltra';
...
<rss version="2.0">
  <channel>
    ...
    <description>Releases of MailBox Ultra. Latest: v${version}.</description>
...
function buildHead({ title, description, slug, version }) {
  const fullTitle = slug === '' ? `${title}` : `${title} · MailBox Ultra`;
  const canonical = `${SITE_URL}/${slug}${slug ? '/' : ''}`;
  const ogImage = `${SITE_URL}/img/screenshot.png`;
  const jsonLd = slug === ''
    ? {
  ...
  <meta property="og:site_name" content="MailBox Ultra" />
  <meta name="twitter:card" content="summary_large_image" />
  <meta name="twitter:title" content="${escapeHtml(fullTitle)}" />
  <meta name="twitter:description" content="${escapeHtml(description)}" />
  <meta name="twitter:image" content="${escapeHtml(ogImage)}" />
  <link rel="alternate" type="application/rss+xml" title="MailBox Ultra changelog" href="${escapeHtml(BASE + '/changelog.xml')}" />
